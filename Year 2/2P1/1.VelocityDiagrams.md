...
To find the instantaneous centre of a slider, extend the idea of the slider until it is over the pivot. Draw the imaginary line perpendicular to the rod, acting from the pivot.
The angular velocity of the rod is the same as the angular velocity of the slider
...
If the rods aren't massless we would have to consider the KE of the rod. The best way to do this is with D'Alembert forces
...
To set up the acceleration image we must find expressions for $\textbf{r}_{A/B}$ using polar coordinates.
If there is a slider, define one point on the cylinder connected to one bar, and another on the piston connected to the other bar. These two points are not a rigid connection.
...
Sum of the d'Alembert forces X velocities, plus the sum of d'Alembert torques X angular velocities
...
But if AB is starting rest: $\ddot{\textbf{r}} = r\ddot{\theta}\textbf{e}*$, dramatically simplifying the construction of an acceleration diagram
If the system is starting from rest, you should still assume there's an $\omega$, keep it in the picture, as it's useful and will cancel out at the end.
...
This can be considered as a distributed load $\omega(x)$ on a beam

We find the shear force by taking a cut, preferably so that we include a free end with zero shear force and zero bending moment by default. Then we draw our d'Alembert forces, remembering that the bar is now shorter. 
For example we use $m_{AB}r_{G'}\ddot{\theta}$ as one of the d'Alembert forces

The location of maximum bending moment occurs when the shear force is zero.
...
$\omega$ is just the spin of the body. Be very careful with vector directions and cross products!
