...
Taking a finite set of samples can introduce discontinuities at the start and the end of the data frame, which results in spurious high frequency components. To reduce this effect we use a window function, which tapers smoothly to zero near $n=0 \quad \text{and} \quad n=N-1$
$f_w[n]=f[n]w[n]$
