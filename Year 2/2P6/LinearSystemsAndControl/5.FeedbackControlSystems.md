...
From this we can see that the final value of the step response = transfer function evaluated at $s=0$. This final value is the steady state gain or DC gain, $\lim_{s\rightarrow 0}=G(0)$
...
Nyquist's Stability Theorem allows us to deduce closed-loop properties from open loop properties, the frequency response of the return ratio.
...
This is equivalent to saying that the closed loop transfer function is stable if all the poles of $\frac{L(s)}{1+L(s)}$ (the roots of $1+L(s)=0$ lie in the LHP)
...
The sensitivity needs to be less than 1 up to $\omega_1$, the lowest frequency at which $|S(j\omega)|=1$. The closed loop system will respond to reference inputs at frequencies up to around $\omega_2$, as evident from the plot of the complementary sensitivity, $T(s)$. 
$S(s)+T(s)=1$
